...
- **Checklists e Versionamento:** Deve dar 'check' nas Tarefas (`[x]`) em `.agents/docs/tasks/` e **Obrigatoriamente** invocar a skill `git-expert` para gerenciar a criação da sua branch de desenvolvimento e para formatar os seus commits (ex: atrelando as mudanças aos IDs `P-XXX`).
