...
- A normalização de texto deve:
  - Converter para minúsculas (`lower()`).
  - Remover acentos diacríticos (ex: "trânsito" -> "transito", "guerra" -> "guerra").
  - Remover pontuações, caracteres especiais e emojis redundantes.
  - Colapsar espaços múltiplos em um espaço único.
  - Remover stopwords insignificantes no início da frase (ex: "por favor", "me diz", "você sabe").
...
- Se o backend de cache (L1 memória ou L2 Firestore) falhar ou lançar exceção, o fluxo principal de processamento de IA **NÃO PODE SER INTERROMPIDO**. O sistema deve registrar warning no log e prosseguir normalmente com a chamada ao Gemini.

---

### RN-CACHE-006: Métricas e Transparência
- O serviço de cache deve monitorar:
  - `hits`: Quantidade de requisições respondidas via cache.
  - `misses`: Quantidade de requisições que precisaram chamar a LLM.
  - `tokens_saved_estimated`: Estimativa acumulada de tokens poupados (baseado na contagem aproximada de caracteres do prompt + resposta / 4).
