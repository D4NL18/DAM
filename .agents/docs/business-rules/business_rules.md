...
* **P-0110 (Governança e Permissões de Calendário em Edição/Exclusão):** Usuários convidados só possuem permissão para editar e excluir eventos da sua própria agenda. Tentativas de alterar a agenda de outros usuários ou do Administrador devem ser bloqueadas com mensagem de segurança. O Administrador pode gerenciar sua própria agenda e gerenciar eventos na agenda compartilhada autorizada mediante especificação explícita (`usuario='user_secundario'`).
