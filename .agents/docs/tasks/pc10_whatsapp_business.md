...
- **Número do Bot:** `+55 71 98171-8497`
- **Número Pessoal Autorizado:** `71 99126-9995` (`5571991269995`)

## 2. Regras de Negócio e Requisitos de Segurança
1. **RN-01 (Isolamento Inviolável):** O webhook FastAPI deve processar EXCLUSIVAMENTE mensagens vindas do número pessoal cadastrado em `ALLOWED_PHONE_NUMBER=5571991269995`. Qualquer mensagem recebida no WhatsApp Business do bot de outro número (terceiros, spams, clientes acidentais) deve ser sumariamente descartada com status `ignored`, motivo `unauthorized_user`, sem emitir resposta, sem marcar como lida e sem consumir tokens de IA.
...
3. **RN-03 (Tolerância ao 9º Dígito):** O número pessoal `71991269995` pode chegar do WhatsApp como `5571991269995@s.whatsapp.net` ou `557191269995@s.whatsapp.net`. A função `is_allowed_user` já tolera isso e deve ser rigorosamente testada.
4. **RN-04 (Configuração de Instância e Reconexão):** O script `conectar_whatsapp.py` deve realizar o logout da sessão anterior (que estava vinculada ao Daniel Marinho no número pessoal) e gerar um QR Code limpo para ser escaneado com o aplicativo WhatsApp Business do número `+55 71 98171-8497`.
5. **RN-05 (Configurações de Ambiente):** Registrar `BOT_PHONE_NUMBER=5571981718497` como referência opcional de documentação em `settings.py` e `.env.example`, mantendo `ALLOWED_PHONE_NUMBER=5571991269995`.
...
- [ ] Atualizar testes unitários em `backend_ia/tests/test_webhook_isolation.py` cobrindo:
  ...
  - Aceitação da mensagem vinda de `5571991269995` (e com oscilação do 9º dígito).
