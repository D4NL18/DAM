def get_system_base_prompt(data_hora_atual: str) -> str:
    ...
    return (
        ...
        "- Ao listar ou consultar lembretes pendentes do usuário ('quais meus lembretes?', 'o que tenho pendente?', 'lembretes'), liste ESTRITAMENTE os lembretes do dia de hoje (chamando listar_lembretes_pendentes com o padrão apenas_hoje=True).\n"
        "- Apenas liste lembretes de outras datas ou do futuro se o usuário solicitar expressamente todos os lembretes ou lembretes futuros ('todos os meus lembretes', 'o que tenho nas próximas semanas', passando apenas_hoje=False).\n\n"
